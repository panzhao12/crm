...
function contactKey(contact: Pick<Contact, 'profileUrl' | 'email' | 'name' | 'company'>): string {
  if (contact.profileUrl) return `url:${normalizeLinkedInUrl(contact.profileUrl)}`;
...
function newPlaceholderContact(profileUrl: string, group = '', tags: string[] = []): Contact {
  ...
  return {
    ...
    firstName: '',
    lastName: '',
    name: normalized ? normalized.replace('https://www.linkedin.com/in/', '').replace(/\/$/, '') : 'New contact',
    headline: '',
    company: '',
    position: '',
    location: '',
...
export const useContactsStore = defineStore('contacts', () => {
  ...
  const selectedContact = computed(() => contacts.value.find((contact) => contact.id === selectedId.value));

  const groups = computed(() =>
    [...new Set(contacts.value.map((contact) => contact.group || contact.category).filter(Boolean))].sort()
  );
  const categories = groups;

  const tags = computed(() =>
    [...new Set(contacts.value.flatMap((contact) => contact.tags).filter(Boolean))].sort()
  );
  ...
  const filteredContacts = computed(() => {
    ...
    return contacts.value.filter((contact) => {
      const matchesSearch =
        !query ||
        [contact.name, contact.headline, contact.company, contact.location, contact.email, contact.notes]
          .join(' ')
          .toLowerCase()
          .includes(query);
      const matchesCategory = !categoryFilter.value || (contact.group || contact.category) === categoryFilter.value;
      const matchesTag = !tagFilter.value || contact.tags.includes(tagFilter.value);
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      if (!selectedId.value && contacts.value[0]) selectedId.value = contacts.value[0].id;
    } finally {
      loading.value = false;
    }
  }
  ...
  async function importCsv(text: string): Promise<CsvImportResult> {
    ...
    const imported = rows.map(contactFromLinkedInRow).filter((contact): contact is Contact => Boolean(contact));
    const byKey = new Map(contacts.value.map((contact) => [contactKey(contact), contact]));
  ...
  async function createManualContact(profileUrl = '', group = '', tags: string[] = []): Promise<Contact> {
    ...
    const contact = newPlaceholderContact(profileUrl, group, tags);
  ...
  async function bulkImportUrls(urls: string[], group = '', tags: string[] = []): Promise<CsvImportResult> {
    const uniqueUrls = [...new Set(urls.map(normalizeLinkedInUrl).filter(Boolean))];
    const existingKeys = new Set(contacts.value.map((contact) => contactKey(contact)));
    const createdContacts: Contact[] = [];
    let updated = 0;

    for (const url of uniqueUrls) {
      const existing = contacts.value.find((contact) => normalizeLinkedInUrl(contact.profileUrl) === url);
      if (existing) {
    ...
    await load();
    return { created: createdContacts.length, updated, skipped: urls.length - uniqueUrls.length };
  }
  ...
  function findByProfileUrl(url: string): Contact | undefined {
    ...
    return contacts.value.find((contact) => normalizeLinkedInUrl(contact.profileUrl) === normalized);
  }
